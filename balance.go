...
// balance position (-1 full left .. 0 centre .. +1 full right). It sits in the
...
// at centre both channels pass at unity; toward one side the OPPOSITE channel
// fades to silence while the near channel stays at unity. Centre is a true bypass.
package main

import "github.com/gopxl/beep/v2"

// balanceGains converts a balance position (-1..+1) to per-channel linear gains.
// Centre (0) is unity on both; panning right attenuates left and vice versa.
...
func (b *balanceStreamer) Stream(buf [][2]float64) (int, bool) {
	...
	// Centre is unity on both channels; skip the multiply entirely so the common
