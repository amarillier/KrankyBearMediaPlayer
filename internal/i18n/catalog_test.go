...
func TestInitAndLookup(t *testing.T) {
	...
	SetLocale("en")
	if got := T("app.name"); got != "KrankyBear MediaPlayer" {
...
func TestInterpolation(t *testing.T) {
	...
	got := TC("windows.help_title", map[string]string{"app_name": "KrankyBear MediaPlayer"})
	if got != "KrankyBear MediaPlayer - Help" {
...
func TestMergeAndFallback(t *testing.T) {
	...
	// A partial locale: only overrides common.close; everything else falls back to English.
	...
	// Not translated in fr → English fallback.
